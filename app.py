import streamlit as st
import pandas as pd
import numpy as np
import plotly.graph_objects as go
from scipy.stats import norm
import os

# --- PAGE CONFIGURATION ---
st.set_page_config(page_title="Inventory Strategy Optimizer", layout="wide")

# --- DATA LOADING ---
def load_data():
    csv_filename = 'InventoryModelInput.csv'
    df = None
    if os.path.exists(csv_filename):
        df = pd.read_csv(csv_filename)
    else:
        uploaded_file = st.sidebar.file_uploader("Upload Inventory CSV", type="csv")
        if uploaded_file is not None:
            df = pd.read_csv(uploaded_file)
    
    if df is not None:
        # Strip whitespace from column names to prevent mapping errors
        df.columns = df.columns.str.strip()
    return df

# --- CALCULATION FUNCTION ---
def calculate_ss_safe(row, forecast_acc, lt_adj_pct):
    try:
        # Extract values
        sl_raw = row.get('Targeted service level', 0.95)
        daily_vol = pd.to_numeric(row.get('Daily volume', 0), errors='coerce')
        base_lt = pd.to_numeric(row.get('Lead-time to customer (days)', 1), errors='coerce')
        
        # Clean service level strings (e.g., '94%' -> 94)
        if isinstance(sl_raw, str):
            sl_raw = sl_raw.replace('%', '')
        sl_val = pd.to_numeric(sl_raw, errors='coerce')
        
        if pd.isna(sl_val) or pd.isna(daily_vol) or pd.isna(base_lt):
            return 0
        
        # Determine probability (if user entered 95, treat as 0.95)
        prob = sl_val / 100.0 if sl_val > 1.0 else sl_val
        prob = max(0.001, min(0.999, prob)) # Clamp for Z-score stability
        
        z = norm.ppf(prob)
        adj_lt = base_lt * (1 + lt_adj_pct / 100)
        sigma = daily_vol * (1 - (forecast_acc / 100))
        
        ss = z * sigma * np.sqrt(max(0, adj_lt))
        return int(max(0, ss))
    except:
        return 0

# --- UI START ---
st.title("📦 Inventory Strategy & Sensitivity Dashboard")

# --- SIDEBAR ---
st.sidebar.header("Global Levers")
# Default Forecast Accuracy set to 57%
global_fa = st.sidebar.slider("Current Forecast Accuracy (%)", 0.0, 100.0, 57.0, 0.5)
lt_adj = st.sidebar.slider("Lead-Time Adjustment (%)", -100, 200, 0, step=5)

raw_df = load_data()

if raw_df is not None:
    # Initial cleaning for the data editor display
    if 'Targeted service level' in raw_df.columns:
        if raw_df['Targeted service level'].dtype == 'object':
            raw_df['Targeted service level'] = raw_df['Targeted service level'].astype(str).str.replace('%', '', regex=False)
            raw_df['Targeted service level'] = pd.to_numeric(raw_df['Targeted service level'], errors='coerce')

    # Layout: Table/Metrics on left, Graph on right
    col_table, col_graph = st.columns([1.2, 1])

    # Because metrics are ABOVE the table but depend on the table data, 
    # we use the editor's return value to refresh the page.
    with col_table:
        st.subheader("1. Portfolio Strategy")
        
        # Container for metrics to appear at the top
        metric_container = st.container()
        
        # Render the table with high 'height' to show more records (full table)
        # 35px per row + header is a good estimate for height
        table_height = min(2000, (len(raw_df) + 1) * 37)
        
        edited_df = st.data_editor(
            raw_df, 
            num_rows="dynamic", 
            use_container_width=True,
            height=table_height, 
            column_config={
                "Targeted service level": st.column_config.NumberColumn("Service Level (decimal or %)")
            }
        )
        
        # Calculate Safety Stock based on current edits
        edited_df['Safety Stock'] = edited_df.apply(
            lambda r: calculate_ss_safe(r, global_fa, lt_adj), axis=1
        )
        
        # Calculate aggregates
        total_ss = edited_df['Safety Stock'].sum()
        total_vol = pd.to_numeric(edited_df['Daily volume'], errors='coerce').sum()
        lts = pd.to_numeric(edited_df['Lead-time to customer (days)'], errors='coerce')
        avg_lt = lts.mean() * (1 + lt_adj / 100) if not lts.empty else 1

        # Display metrics in the container at the TOP
        with metric_container:
            m1, m2 = st.columns(2)
            m1.metric("Total Network Safety Stock", f"{int(total_ss):,} units")
            m2.metric("Total Daily Portfolio Volume", f"{int(total_vol):,} units")
            st.markdown("---")

        st.download_button("Export Inventory Plan", edited_df.to_csv(index=False), "inventory_plan.csv")

    with col_graph:
        st.subheader("2. Network Sensitivity Analysis")
        
        # Sensitivity Grid
        fa_range = np.linspace(0, 100, 40)
        sl_range = np.linspace(0.80, 0.999, 40) 
        FA, SL = np.meshgrid(fa_range, sl_range)

        # Vectorized Surface Calculation
        Z_grid = norm.ppf(SL)
        Sigma_total = total_vol * (1 - (FA / 100))
        SS_surface = Z_grid * Sigma_total * np.sqrt(max(0.1, avg_lt))

        fig = go.Figure(data=[go.Surface(
            z=SS_surface, x=fa_range, y=sl_range * 100,
            colorscale='Turbo',
            hovertemplate='<b>FA:</b> %{x:.1f}%<br><b>Service:</b> %{y:.1f}%<br><b>Total SS:</b> %{z:,.0f}<extra></extra>'
        )])

        fig.update_layout(
            scene=dict(
                xaxis_title="Forecast Accuracy (%)", 
                yaxis_title="Service Level (%)", 
                zaxis_title="Total SS"
            ),
            margin=dict(l=0, r=0, b=0, t=30), 
            height=700
        )
        st.plotly_chart(fig, use_container_width=True)
else:
    st.info("Please ensure 'InventoryModelInput.csv' is in the root directory or upload it via the sidebar.")


